...
RUN cat tai.txt hua.txt | awk '{print $1}' > pio2.txt
RUN cat ing.txt | egrep -v '^[A-Z]\s' | awk '{print $1}' > ingpio2.txt

RUN /opt/bin/i686-m64/ngram-count -text pio2.txt -order 1 -lm pio2.arpa
RUN /opt/bin/i686-m64/ngram-count -text ingpio2.txt -order 1 -lm ingpio2.arpa
